package com.sgk.model.adapters.constraint_layout
...
class ChildConstraintModelJsonAdapter (
    private val directionConstraintsJsonAdapter: JsonAdapter<com.sgk.model.modal.DirectionConstraints>
):  JsonAdapter<com.sgk.model.modal.ChildConstraintModel>() {

    override fun fromJson(reader: JsonReader): com.sgk.model.modal.ChildConstraintModel? {
        ...
        while (reader.hasNext()) {
            val temData = reader.selectName(com.sgk.model.adapters.AdapterConstants.KEY_OPTIONS_CHILD_CONTRAINT_SET)
            when (temData) {
                0 -> {
...
//                    val element = childConstraintModelJsonAdapter.fromJsonValue(jsonValueMap) ?: break
        ...
        return com.sgk.model.modal.ChildConstraintModel(
            refId, top, bottom, start, end, width, height
        )

    }

    override fun toJson(writer: JsonWriter, value: com.sgk.model.modal.ChildConstraintModel?) {
        if (value == null) {
            writer.nullValue()
        } else {
            writer.beginObject()

            writer.name(com.sgk.model.adapters.AdapterConstants.KEY_REF_ID)
            writer.value(value.refId)

            writer.name(com.sgk.model.adapters.AdapterConstants.KEY_TOP)
            directionConstraintsJsonAdapter.toJson(writer, value.top)

            writer.name(com.sgk.model.adapters.AdapterConstants.KEY_BOTTOM)
            directionConstraintsJsonAdapter.toJson(writer, value.bottom)

            writer.name(com.sgk.model.adapters.AdapterConstants.KEY_START)
            directionConstraintsJsonAdapter.toJson(writer, value.start)

            writer.name(com.sgk.model.adapters.AdapterConstants.KEY_END)
            directionConstraintsJsonAdapter.toJson(writer, value.end)

            writer.name(com.sgk.model.adapters.AdapterConstants.KEY_WIDTH_CONSTRAINT)
            writer.value(value.width_constraint?.typeString)

            writer.name(com.sgk.model.adapters.AdapterConstants.KEY_HEIGHT_CONSTRAINT)
