...
//        var id: String? = null
        ...
        while (reader.hasNext()) {
            when (reader.selectName(com.sgk.model.adapters.AdapterConstants.KEY_OPTIONS_IMAGE)) {
                0 -> {
...
//            id = id ?: System.currentTimeMillis().toString()
    ...
    override fun toJson(writer: JsonWriter, value: Image?) {
        ...
        writer.name(com.sgk.model.adapters.AdapterConstants.KEY_URL)
        writer.value(value.url)

        writer.name(com.sgk.model.adapters.AdapterConstants.KEY_ALT_TEXT)
        writer.value(value.altText)

        writer.name(com.sgk.model.adapters.AdapterConstants.KEY_IMAGE_TYPE)
        writer.value(value.imageType.typeString)

        writer.name(com.sgk.model.adapters.AdapterConstants.KEY_STYLE)
        styleJsonAdapter.toJson(writer, value.style)

//        writer.name(com.sgk.sduicore.adapters.AdapterConstants.KEY_ID)
//        writer.value(value.id)

        writer.name(com.sgk.model.adapters.AdapterConstants.KEY_TINT)
